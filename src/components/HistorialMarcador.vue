                ...
                <q-card-section>
                  ...
                  <div class="info-grid">
                    <div>
                      ...
                      {{ registroSeleccionado.apellido }}
                    </div>
                    <div>
                      <strong>DNI:</strong> {{ registroSeleccionado.dni }}
                    </div>
                    <div>
                    ...
                    <div>
                      <strong>Barrio:</strong> {{ registroSeleccionado.barrio }}
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      {{
                        formatearFechaCompleta(
                          registroSeleccionado.fecha_modificacion
                        )
                      }}
                    </div>
                    <div>
                      ...
                      {{
                        formatearTipoOperacion(
                          registroSeleccionado.tipo_operacion
                        )
                      }}
                    </div>
                    <div v-if="registroSeleccionado.notas">
                      <strong>Notas:</strong> {{ registroSeleccionado.notas }}
                    </div>
                  </div>
                </q-card-section>
              </q-card>
              ...
              <q-card flat bordered>
                <q-card-section>
                  <div class="text-h6 text-primary text-weight-bold q-mb-md">
                    ...
                    Programas ({{
                      registroSeleccionado.programas_snapshot?.length || 0
                    }})
                  </div>
              ...
              <q-card flat bordered>
                <q-card-section>
                  <div class="text-h6 text-primary text-weight-bold q-mb-md">
                    ...
                    Integrantes ({{
                      registroSeleccionado.integrantes_snapshot?.length || 0
                    }})
                  </div>
              ...
              <q-card flat bordered>
                <q-card-section>
                  <div class="text-h6 text-primary text-weight-bold q-mb-md">
                    ...
                    Servicios ({{
                      registroSeleccionado.servicios_snapshot?.length || 0
                    }})
                  </div>
              ...
              <q-card flat bordered>
                <q-card-section>
                  ...
                  <div v-for="vivienda in registroSeleccionado.viviendas_snapshot" :key="vivienda.id" class="info-grid">
                    ...
                    <div>
                      <strong>Ambientes:</strong> {{ vivienda.ambientes }}
                    </div>
                    <div>
                      <strong>Baño:</strong> {{ vivienda.baño }} ({{
  ...
  setup(props) {
    ...
    const historialFiltrado = computed(() => {
      ...
      if (filtroFechaInicio.value || filtroFechaFin.value) {
        resultado = resultado.filter((registro) => {
          ...
          const fechaFin = filtroFechaFin.value
            ? new Date(filtroFechaFin.value + 'T23:59:59')
            : null;
      ...
      if (filtroTipoOperacion.value) {
        resultado = resultado.filter(
          (registro) => registro.tipo_operacion === filtroTipoOperacion.value
        );
      }
    ...
    const cargarHistorial = async () => {
      ...
      try {
        ...
        const historialCompleto = response.data || [];
        historial.value = historialCompleto.filter(
          (registro) => registro.tipo_operacion === 'BEFORE_RELATIONS_UPDATE'
        );

        // Extraer tipos de operación únicos para el filtro (solo será pre-actualización)
        const tiposUnicos = [
          ...new Set(historial.value.map((item) => item.tipo_operacion)),
        ];
